...
echo ""
echo -e "${CYAN}🔧 Preparing Azure Function for VMSS auto-shutdown...${NC}"
echo -e "${CYAN}Function App: ${YELLOW}$FUNCTION_APP_NAME${NC}"
echo -e "${CYAN}Note: Robust VMSS function will be deployed in post-deployment tasks${NC}"
...
./post-deployment-tasks.sh "$RESOURCE_GROUP" "$REDHAT_VM_NAME" "$UBUNTU_VM_NAME" "$WINDOWS_VM_NAME" "$VMSS_NAME" "$REDHAT_PRIVATE_IP" "$USER_TIMEZONE" "$FUNCTION_APP_NAME"
...
echo -e "${CYAN}  - Robust VMSS shutdown function deployed to Azure Function${NC}"
